...
lazy_static! {
    // current version and author of Engine
    pub static ref NAME: String = String::from("Eden 1.0");
    pub static ref AUTHOR: String = String::from("jScheinert, cBrendel");

    // get conversion tables between 12x12 and 8x8 boards
    pub static ref CONV_TUP: ([usize; 120], [usize; 64]) = {
        let mut temp_b_t_s: [usize; 120] = [64; 120];
        let mut temp_s_t_b: [usize; 64] = [120; 64];
        ...
        (temp_b_t_s, temp_s_t_b)
    };
    pub static ref BIG_TO_SMALL: [usize; 120] = CONV_TUP.0;
    pub static ref SMALL_TO_BIG: [usize; 64] = CONV_TUP.1;

    // random keys for position hash
    pub static ref PIECE_KEYS: [[u64; 13]; 120] = {
        let mut piece_keys: [[u64; 13]; 120] = [[0; 13]; 120];
    
            for outer in 0..120 {
                for inner in 0..13 {
                    piece_keys[outer][inner] = rand::random::<u64>();
                }
            }
            return piece_keys;
    };
    pub static ref SIDE_KEY: u64 = rand::random::<u64>();
    pub static ref CASTLING_KEYS: [u64; 16] = {
    ...
    pub static ref START_FEN: String = String::from("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");

    // conversion table from 120 board to ranks/files of 64 board
    pub static ref FILES_FROM_120: [usize; 120] = {
        let mut files_from_120: [usize; 120] = [Files::NoF as usize; 120];
        let mut sq: usize;

        for rank in 0..8 {
            for file in 0..8 {
                sq = (21 + file) + (rank * 10);
                files_from_120[sq] = file;
            }
        }
        return files_from_120;
    };
    pub static ref RANKS_FROM_120: [usize; 120] = {
        let mut ranks_from_120: [usize; 120] = [Ranks::NoR as usize; 120];
        let mut sq: usize;

        for rank in 0..8 {
            for file in 0..8 {
                sq = (21 + file) + (rank * 10);
                ranks_from_120[sq] = rank;
            }
        }
        return ranks_from_120;
    };

    // "Most Valuable Victim - Least Valuable Attacker" pairing
    pub static ref MVV_LVA: [[usize; 13]; 13] = {
        let mut mvv_lva: [[usize; 13]; 13] = [[0; 13]; 13];

        for attacker_u in 1..13 {
            for victim_u in 1..13 {
                mvv_lva[victim_u][attacker_u] = 100 * VICTIM_SCORES[victim_u] + 6 - VICTIM_SCORES[attacker_u];
            }
        }
    ...
    //for each square on the board build an appropriate set and clear mask
    pub static ref SET_MASK: [u64; 64] = {
        let mut set_mask: [u64; 64] = [0; 64];

      for index in 0..64 {
            set_mask[index] = lsh(1,index);
        }
       set_mask
    };
...
pub static mut TRANS_TABLE_SIZE: usize = 16 * 0x100000;

// char arrays for visualization
...
pub static PIECE_CHARS:  [char; 13] = ['.','P','N','B','R','Q','K','p','n','b','r','q','k'];  //['.','♙','♘','♗','♖','♕','♔','♟','♞','♝','♜','♛','♚'];


// masks to describe features of pieces
pub static BIG_MASK:                [bool; 13] = [false, false, true,  true,  true,  true,  true,  false, true,  true,  true,  true,  true ];
pub static MAJ_MASK:                [bool; 13] = [false, false, false, false, true,  true,  true,  false, false, false, true,  true,  true ];
pub static MIN_MASK:                [bool; 13] = [false, false, true,  true,  false, false, false, false, true,  true,  false, false, false];
pub static IS_PAWN_MASK:            [bool; 13] = [false, true,  false, false, false, false, false, true,  false, false, false, false, false];
pub static IS_KNIGHT_MASK:          [bool; 13] = [false, false, true,  false, false, false, false, false, true,  false, false, false, false];
pub static IS_BISHOP_QUEEN_MASK:    [bool; 13] = [false, false, false, true,  false, true,  false, false, false, true,  false, true,  false];
pub static IS_ROOK_QUEEN_MASK:      [bool; 13] = [false, false, false, false, true,  true,  false, false, false, false, true,  true,  false];
pub static IS_KING_MASK:            [bool; 13] = [false, false, false, false, false, false, true,  false, false, false, false, false, true ];                                      
pub static COLOR_MASK:           [Players; 13] = [Players::Both,
...
// values of pieces as per AlphaZero:
pub static VALUES: [usize; 13] = [0, 100, 305, 333, 563, 950, 2222,
                                     100, 305, 333, 563, 950, 2222];

// possible directions for each piece to move to                                     
pub static KNIGHT_DIRECS:           [i8; 8] = [-8, -19, -21, -12, 8, 19, 21, 12];
pub static BISHOP_DIRECS:           [i8; 4] = [-9, -11, 11, 9];
pub static BISHOP_DIRECS_FILLED:    [i8; 8] = [-9, -11, 11, 9, 0, 0, 0, 0];
pub static ROOK_DIRECS:             [i8; 4] = [-1, -10, 10, 1];
pub static ROOK_DIRECS_FILLED:      [i8; 8] = [-1, -10, 10, 1, 0, 0, 0, 0];
pub static KING_DIRECS:             [i8; 8] = [-1, -10, 10, 1, -9, -11, 11, 9];



// XOR flags for move attributes
pub static EP_FLAG:   usize = 0x400000;
pub static X2_FLAG:   usize = 0x800000;
pub static CAST_FLAG: usize = 0x1000000;
pub static CAP_FLAG:  usize = 0x43C000;


//constants for alpha beta
pub static INF: i32 = 50000;
pub static MATE: i32 = 30000; //30000 pm (moves to mate) lets Arena know how far away the mate is
pub static IS_MATE: i32 = 29936; // if mate is found then it will be in less then 64 plies


// victim scores for move ordering heuristic
pub static VICTIM_SCORES: [usize; 13] = [0, 1, 2, 3, 4, 5, 6, 1, 2, 3, 4, 5, 6];

                                     
// array to update castle permissions                                         
pub static CASTLE_PERM: [u8; 120] = [
...
// array of usizes corresponding to pieces
pub static WHITE_USIZES: [usize; 6] = [Pieces::Wp as usize, Pieces::Wn as usize, Pieces::Wb as usize,
                                       Pieces::Wr as usize, Pieces::Wq as usize, Pieces::Wk as usize];                                                  
pub static BLACK_USIZES: [usize; 6] = [Pieces::Bp as usize, Pieces::Bn as usize, Pieces::Bb as usize,
                                       Pieces::Br as usize, Pieces::Bq as usize, Pieces::Bk as usize];


// tables of (un-)valuable places for pieces to stand
pub static PAWN_POS_VALS: [i32; 64] = [
    0	,	0	,	0	,	0	,	0	,	0	,	0	,	0	,
...
pub static PAWN_POS_VALS_ENDGAME: [i32; 64] = [
    0	,	0	,	0	,	0	,	0	,	0	,	0	,	0	,
   -10	,  -10	,  -5	,	0	,	0	,  -5	,  -10	,  -10	,
   -5	,  -5	,	0	,	5	,	5	,	0	,  -5	,  -5	,
    0	,	0	,	0	,	5	,	5	,	0	,	0	,	0	,
...
pub static KNIGHT_POS_VALS: [i32; 64] = [ // good for endgame aswell
   -15	,	-10	,	0	,	0	,	0	,	0	,	-10	,  -15	,
   -10	,	0	,	0	,	5	,	5	,	0	,	0	,  -10	,
...
pub static BISHOP_POS_VALS: [i32; 64] = [ // good for endgame aswell
    0	,	0	,	-10	,	0	,	0	,	-10	,	0	,	0	,
    0	,	10	,	0	,	10	,	10	,	0	,	10	,	0	,
    0	,	0	,	10	,	15	,	15	,	10	,	0	,	0	,
...
pub static ROOK_POS_VALS: [i32; 64] = [ // good for endgame aswell
    0	,	0	,	5	,	10	,	10	,	5	,	0	,	0	,
    0	,	0	,	5	,	10	,	10	,	5	,	0	,	0	,
    0	,	0	,	5	,	10	,	10	,	5	,	0	,	0	,
    0	,	0	,	5	,	10	,	10	,	5	,	0	,	0	,
...
pub static QUEEN_POS_VALS: [i32; 64] = [ // good for endgame aswell
   -10  ,   0	,  -10	,  -5	,  -5	,  -5	,  -10	,  -20	,
...
pub static KING_POS_VALS: [i32; 64] = [
...
pub static KING_POS_VALS_ENDGAME: [i32; 64] = [
...
// mirror array as above tables are only for white
pub static MIRROR: [usize; 64] = [
...
// polyglot format of pieces for promotion
pub static PIECE_TO_POLY_KIND: [usize; 13] = [0 /* index 0 value ignored */, 1, 3, 5, 7, 9, 11, 0, 2, 4, 6, 8, 10];

// found at http://hgm.nubati.net/book_format.html taken from Polyglot source code
// predetermined Zobrist keys from Polyglot source code to allow compatibility with Polyglot books
pub static POLY_ZOBRIST_KEYS: [u64; 781] = [
